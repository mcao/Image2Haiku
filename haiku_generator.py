...
def main_shit(word_list, final_senti):
    ...
    dicti = dictionary(word_list)
    
    finding_nouns_n_shit(dicti)
    ...
    third_line_list  = []
    for i in range(5):
        first_line_list.append(first_line_generator(adj_dict, noun_dict, verb_dict))
        second_line_list.append(sec_line_generator(adj_dict, noun_dict, verb_dict))
        third_line_list.append(third_line_generator(adj_dict, noun_dict, verb_dict))
    ...
    final_senti = black_magic(res,emo) #must fix with mike
    small = 0
